...
* Because of the model definition, after a certain market access level (~1900), yields become negative. To correct for this, if the yield_response function returns a negative or zero value, the output of the function will be NA.
...
ranges[,1] <- c( 'ZERO','BK', 'OPyield', 'OPnetrev','BK_gain_perc','OPyield_gain_perc','OPnetrev_mean_gain_perc') 
...
for (s in scenarios){
  ...
  ranges[r, 11] <- 100 * sum(results$netrev<=low_netrev, na.rm = TRUE)/maize_area
  ranges[r, 12] <- 100 * sum(results$netrev>=high_netrev, na.rm = TRUE)/maize_area
...
for (s in scenarios){
  ...
  value <- cbind(s, yield=results$yield_mean) %>% as_tibble()
...
ranges <- read.csv('../results/tables/table1_TZA_Ranges.csv') %>% 
  filter(Scenario %in% c("ZERO", "BK_gain_perc", "OPyield_gain_perc", "OPnetrev_gain_perc")) %>% 
  dplyr::select(Scenario, yieldMean) %>% 
  mutate(label = paste0(yieldMean, "%"))
ranges$label[1] <- "0%"
ranges$s <- factor(scenarios, levels=scenarios)

p <- ggplot(values, aes(x=s, y=yield, fill=s)) + 
...
ggsave(filename = paste0('../results/plots/FigureX_yield_boxplot.tif'),
      plot = p,
      width = 3, height = 3, units = 'in',
      device = 'tiff', dpi = 300)
...
p <- ggplot(values, aes(x=s, y=netrev, fill=s)) + 
...
ggsave(filename = paste0('../results/plots/FigureX_netrev_boxplot.tif'),
      plot = p,
      width = 3, height = 3, units = 'in',
      device = 'tiff', dpi = 300)
