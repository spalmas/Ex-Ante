...
TZAPS_subset <- TZAPS %>% dplyr::select(yld, N_kgha, intercrop, rotat1, manure,
                                        ...
                                        af_ORCDRC_T__M_sd1_1000m_TZA,#Longitude, Latitude,
                                        af_PHIHOX_T__M_sd1_1000m_TZA,
...
plot_fitness(obs = TZAPS_subset$yld,
             pred = pred,
             "Fitness")
```
...
```{r}
TZAPS %>% summarise(year = mean(year, na.rm=TRUE),
                    intercrop = mean(intercrop, na.rm=TRUE),
                    rotat1 = mean(rotat1, na.rm=TRUE),
                    manure = mean(manure, na.rm=TRUE),
                    cropres = mean(cropres, na.rm=TRUE),
                    weedings = mean(weedings, na.rm=TRUE),
                    impseed = mean(impseed, na.rm=TRUE),
                    disease = mean(disease, na.rm=TRUE),
                    striga = mean(striga, na.rm=TRUE),
                    fallow3yr = mean(fallow3yr, na.rm=TRUE),
                    struct = mean(struct, na.rm=TRUE),
                    terraced = mean(terraced, na.rm=TRUE),
                    logha = mean(logha, na.rm=TRUE),
                    headage = mean(headage, na.rm=TRUE),
                    femhead = mean(femhead, na.rm=TRUE),
                    hhsize = mean(hhsize, na.rm=TRUE),
                    headeduc = mean(headeduc, na.rm=TRUE)) %>% t()
```